...
async function editarDocente(id){
    ...
    const id_especialidad = docenteTemp.id_especialidad.id_especialidad;
...
function desactivarEstudiante(id){
    $("#btnDesactivarDocente").click(function() {
        $.ajax({
            type: "PUT",
...
function crearListaDocentes(docente) {
    ...
    btnDelete.forEach(boton => {
        boton.addEventListener("click", function() {
            //const idDocente = boton.dataset.idUsuario;
            const idUsuario = boton.dataset.idUsuario;
            desactivarEstudiante(idUsuario);
        });
    });
}
